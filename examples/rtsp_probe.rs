...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("Usage: rtsp_probe <rtsp_uri> [seconds=8] [width=1280] [height=720]");
        std::process::exit(2);
    }
    ...
    while start.elapsed().as_secs_f64() < run_seconds {
        ...
        if first_frame_elapsed.is_none() && count > 0 {
            ...
            println!(
                "[rtsp_probe] first_frame={:.3}s frames={}",
    ...
    println!(
        "[rtsp_probe] done uri={} seconds={:.2} frames={}",
